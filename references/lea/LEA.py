...
class LEA(object):
    def __init__(self, key):
        ...
        if key_size == SIZE_128:
            ...
            raise AttributeError(f'Improper key size: {key_size}')

        self.keySize = (SIZE_128, SIZE_192, SIZE_256)
        self.delta = [0xc3efe9db, 0x44626b02, 0x79e27c8a, 0x78df30ec, 0x715ea49e, 0xc785da0a, 0xe04ef22a, 0xe5c40957]

        # LEA delta
        T = [0] * 8
        mk_len = len(key)
        mk = LEA.to_bytearray(key)
        self.rk = [[0 for _ in range(6)] for _ in range(32)]
        ...
        if mk_len == 16:
            for i in range(0, self.rounds, 1):
                ...
                self.rk[i][1] = self.rk[i][3] = self.rk[i][5] = T[1] = self.ROL((T[1] + self.ROL(temp, 1)) & 0xffffffff,
                                                                                3)
    ...
    def encrypt(self, plaintext):
        if len(plaintext) != 16:
            raise AttributeError('length of plaintext should be 16 not %d' % len(plaintext))

        # plaintext = LEA.to_bytearray(plaintext)
        temp = list(struct.unpack('<LLLL', plaintext))
